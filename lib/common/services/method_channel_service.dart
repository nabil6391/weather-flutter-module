...
class MethodChannelService {
    ...
    _channel.setMethodCallHandler((call) async {
      ...
      if (call.method == Constants.getUserProfileMethod) {
        if (call.arguments is Map) {
          ...
          debugPrint(
              'MethodChannelService: Received invalid arguments type for ${Constants.getUserProfileMethod}');
        }
      }
    });
  }
  ...
  static Future<void> sendFeedback(String message) async {
    try {
      await _channel
          .invokeMethod(Constants.sendFeedbackMethod, {'feedback': message});
      ...
      if (e.code == 'MethodNotImplemented') {
        debugPrint(
            'Native method ${Constants.sendFeedbackMethod} not implemented.');
      }
    } catch (e) {
